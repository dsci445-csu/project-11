...
```{r}
#Fit PCR
m0 <- pcr(price~., data = training,ncomp= 25, validation = "CV")
mse <- MSEP(m0)
data.frame(M = mse$comps, mse = t(as.data.frame(mse$val))[, "CV"]) %>%
ggplot() +
  geom_line(aes(M, mse)) +
  geom_point(aes(M, mse))
# based off of plot we will select 20 components (elbow method) = 99.9% of variance explained
mse$val
summary(m0)

# R^2 vs Principal Compenets Plot
validationplot(m0, val.type = "R2")

# Test Set Predictions
pcr_pred <- predict(m0, testing, ncomp = 20)

# Prediction Plot
predplot(m0, ncomp = 20)
...
```{r}
# Fit PLS
m1 <- plsr(price~., data=training, validation="CV",ncomp=25)
...
fit0 <- glm(price ~ bs(accommodates, df = 2), data = training_df)
cvs0 <- cv.glm(training_df, fit0, K = 10)$delta[1]
...
cvs2 <- cv.glm(training_df, fit2, K = 10)$delta[1]

fit3 <- glm(price ~ bs(accommodates, df = 5), data = training_df)
cvs3 <- cv.glm(training_df, fit3, K = 10)$delta[1]
...
#spline on bathrooms
fit <- glm(price ~ bs(bathrooms, df = 3), data = training_df)
cvs <- cv.glm(training_df, fit, K = 10)$delta[1]

fit2 <- glm(price ~ bs(bathrooms, df = 4), data = training_df)
cvs2 <- cv.glm(training_df, fit2, K = 10)$delta[1]

fit3 <- glm(price ~ bs(bathrooms, df = 5), data = training_df)
cvs3 <- cv.glm(training_df, fit3, K = 10)$delta[1]
...
#spline on bedrooms
fit <- glm(price ~ bs(bedrooms, df = 3), data = training_df)
cvs <- cv.glm(training_df, fit, K = 10)$delta[1]

fit2 <- glm(price ~ bs(bedrooms, df = 4), data = training_df)
cvs2 <- cv.glm(training_df, fit2, K = 10)$delta[1]
```
...
```{r}
#fit GAM
gammod <- lm(price ~ . + bs(accommodates, df = 4) + bs(review_scores_rating, df = 3) + bs(bathrooms, df = 3) + bs(bedrooms, df = 4), data = training_df)
summary(gammod)
```
...
# k = 5
# folds = sample(seq_len(k), nrow(training_df) , replace = TRUE)
# results_kfold_bagging = c()
# for(i in seq_len(k)){
#   trn <- folds != i
#   
#   #fit model
#   l <- randomForest(price ~ ., data = training_df[trn, ], mtry = ncol(training_df[trn, ]) - 1, importance = TRUE)
#   
#   # predict on validation set
#   pred = predict(l, training_df[!trn, ])
#   
...
#   true_price = training_df[!trn, ]$price
#   
#   results_kfold_bagging[i] = (true_price - pred)^2
# }
# 
# print(paste("k-fold Cross Validation of Bagging: ", round(mean(results_kfold_bagging), 2)))
```
...
## MSE Table
```{r}
method = c("Linear Regression", "PCR", "Splines", "GAM", "Trees", "Bagging", "Random Forest", "Boosting")
MSEnumbers = c(linearReg_kfold_MSE, 0, csv2, 0, tree_MSE, bagMSE, rf_MSE, boosted_MSE)

finalMSE = data.frame(Methods = method, MSE = MSEnumbers)
finalMSE
